...
bool CDirectInput::Create_Device(GUID id_disp, DWORD Flags) 
{
	...
	if (FAILED(hr=lpdi->CreateDevice(id_disp, &lpdidev,  NULL))) {
	...
	if (FAILED(hr=lpdidev->Acquire())) {
		MessageBox(NULL, "No puc adquirir el dispositiu [CDirectInput::CreateDevice]", "nfo", MB_OK|MB_ICONINFORMATION);
		if (hr==DIERR_OTHERAPPHASPRIO)
			MessageBox(NULL, GetDIError(hr), "[CDirectInput::CreateDevice] other app has prio", MB_OK|MB_ICONINFORMATION);
		return (false);
	}
